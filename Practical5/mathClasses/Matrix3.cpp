...
Matrix3::Matrix3(Vector3 vOne, Vector3 vTwo, Vector3 vThree)
{
	m_11 = vOne.GetX();
	m_12 = vOne.GetY();
	m_13 = vOne.GetZ();
...
//Row Getter Function
Vector3 Matrix3::GetCol(int i) const
{
	switch (i)
	{
	case 0:
...
//'*' Operator Overload
Vector3 Matrix3::operator*(Vector3 &vIn) const
{
...
{
	Vector3 rowOne(m_11, m_21, m_31);
	Vector3 rowTwo(m_11, m_21, m_31);
	Vector3 rowThree(m_13, m_23, m_33);
...
Vector3 Matrix3::RotateAboutX(Vector3 vectorToRotate, float angle)
{
	...
	Vector3 answer = rotationalMatrixX * vectorToRotate;
	return answer;
}

//Function Rotates A Vector About The Y Axis
Vector3 Matrix3::RotateAboutY(Vector3 vectorToRotate, float angle)
{
	double angleInRadians = (angle / 180.0) * acos(-1);
	...
	Vector3 answer = rotationalMatrixY * vectorToRotate;
	return answer;
}

//Function Rotates A Vector About The Z Axis
Vector3 Matrix3::RotateAboutZ(Vector3 vectorToRotate, float angle)
{
	double angleInRadians = (angle / 180.0) * acos(-1);
	...
	Vector3 answer = rotationalMatrixZ * vectorToRotate;
	return answer;
}